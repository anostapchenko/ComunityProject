...
- В основном файле application.properties добавить строку **spring.profiles.active=<имя профиля>**. Где <имя профиля> нужно изменить на имя профиля в котором нужно работать (***spring.profiles.active=local***) 
- Теперь все локальные настройки держим отдельно от серверных.
- Также при разделении на профили можно используя аннотацию @Profile("local") указать спрингу что нужно запускать с нашим профилем. Также можно использовать нотацию @Profile("!local"), т.е. что не нужно запускать в локальном профиле.  
