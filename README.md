...
- Внести в Override parameters данные переменных DB_HOST, DB_PORT, DB_NAME, DB_USERNAME, DB_PASSWORD и SERVER_PORT.
...
- Тесты создаются согласно рест контроллерам, например если есть **ResourceAnswerController**, то есть тест **TestResourceAnswerController** и тестируются все api из контроллера.
...
- На каждый класс тестов написаны отдельные датасеты, в случае, где их нужно изменить, применяется п.5. Например, если мы тестируем ResourceAnswerController, есть подпакет dataset/[название пакета] и тут лежат все датасеты нужные для тестирования этого контроллера.
...
- Все классы для тестов находятся в папке .../api
